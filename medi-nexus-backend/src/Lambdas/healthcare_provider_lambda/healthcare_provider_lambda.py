...
def getHealthCareProvider(event):
    try:
        ...
        KeyConditionExpression = boto3.dynamodb.conditons.Key('providerID').eq(providerID) 
        )
...
def updateHealthCareProvider(event):
    try:
        ...
        response = table.query(
            KeyConditionExpession = boto3.dynamodb.conditions.Key('providerID').
            eq(providerID),
...
def deleteHealthCareProvider(event):
    try:
        ...
        # Get the item from the table
        response = table.query(
          KeyConditionExpression = boto3.dynamodb.conditons.Key('providerID')
          .eq(providerID),
